...
def transfer_data_to_pl(number_of_samples,file_name,repeated_read):
  if file_name != "":
    ...
    with open(file_name, "rb") as f:
      data = f.read()
      ddr_mem.write(data)
  # Turn off DMA and wait for the last transfer to complete
  axil_reg.write_reg(DMA_CTRL_REG_BASEADDR,4, 0)
  time.sleep(10e-3)
  axil_reg.write_reg(DMA_CTRL_REG_BASEADDR,8, 0)
  ...
  axil_reg.write_reg(DMA_CTRL_REG_BASEADDR,4, 0x1 + (repeated_read <<1))
  axil_reg.write_reg(DMA_CTRL_REG_BASEADDR,4, 0x0 + (repeated_read <<1))

def start_tx_mem_load(number_of_samples):
  # Turn off DMA and wait for the last transfer to complete
  # Keep Tx path as 2 to empty fifo data
  axil_reg.write_reg(DMA_CTRL_REG_BASEADDR,4, 0)
  axil_reg.write_reg(TOP_REG_BASEADDR,3, 2)
  time.sleep(10e-3)
  axil_reg.write_reg(TOP_REG_BASEADDR,3, 0)
...
def tx_from_file_mem(number_of_samples,file_name,tx_mode):
  start_tx_mem_load(number_of_samples)
  transfer_data_to_pl(number_of_samples,file_name,False)
  start_tx_mem_play(number_of_samples,tx_mode)
  axil_reg.write_reg(TOP_REG_BASEADDR,3, 0)

def tx_from_file_direct_dma(number_of_samples,file_name,tx_mode):
  transfer_data_to_pl(number_of_samples,file_name,tx_mode)
  axil_reg.write_reg(TOP_REG_BASEADDR,3, 2)

def tx_from_ddr_direct_dma(number_of_samples,tx_mode):
  transfer_data_to_pl(number_of_samples,"",tx_mode)
  axil_reg.write_reg(TOP_REG_BASEADDR,3, 2)